...
var GridView = React.createClass({
	...
	render : function () {
		...
		_.each(this.state.headerLabels, function (label, i) {
			headers.push(<div key={"header-" + i} style={{
				position : "absolute",
				top : 0,
		...
		_.each(this.props.data, function(row, i){
			...
			_.each(row, function(col, j){
				cells.push(<div data-row={i} data-col={j} style={{
					position : "absolute",
					top : 0,
			...
			table.push(<div style={{
															position : "absolute",
															top : this.state.tops[i],
															width : "100%",
															height : this.state.heights[i],
														}} className={className} key={"row-" + i}>{cells}</div>);
		}, this);

		return (
			<div className="gridView">
				<header ref="header" style={{position : "absolute", width : this.state.width, overflow : "hidden", zIndex : 1, left : this.state.scrollLeft }}>
					{headers}
				</header>
				<section ref="table" style={{position : "absolute", width : "100%", height : "100%", overflow : "auto" }} onScroll={this.onScrollTable}>
					{table}
				</section>
			</div>
		);
