			...
			if (this.showHourMarker(hr)) {
				...
				hours.push(
					<div key={hr} className={(hr % 2 == 0) ? "even hourMarker" : "odd hourMarker"} style={style}>
						<div className="label" style={{ position : "absolute", top : -10, width : 70, left : this.props.width / 2 - 35, textAlign : "center" }}>{(hr <= 12) ? hr + ":00am" : (hr - 12) + ":00pm"}</div>
					</div>
				);
			}
		}
		...
		for (var i=0, d; d=this.props.data[i]; i++) {
			...
			if (!allDay) {
				if (startDate.valueOf() > this.state.monday.valueOf() && startDate.valueOf() < this.state.sunday.valueOf()) {
					...
					data.push(<div key={"data-" + i} className="entry" style={style} data-index={i} onClick={this.onSelectData}>{this.dataTitle(d,i)}</div>);
				}
			} else {
	...
	renderTimeRuler : function () {
		...
		return (
			<div className="timeRuler" style={{ position : "absolute", left : 0, height : 0, width : this.props.width, top : this.state.timeRuler.y }}>
				<div className="label">{Time.timeString(this.state.timeRuler.time)}</div>
			</div>
		);
	},
	renderPopover : function () {
		if (!this.state.popover) { return undefined; }
		return <this.props.popover {...this.props} x={this.state.popover.x} y={this.state.popover.y} position="top" onClose={this.onClosePopover} date={this.state.popover.date}/>
	},
	render : function () {
		...
		for (var d=0; d < 7; d++) {
	 	...
	 	return(
	 		<div ref="container" className="calendar week" style={{ position : "absolute", top : 0, left : 0, width : this.props.width }}>
	 			...
	 			<div ref="viewport" className="viewport" style={viewportStyle} 
